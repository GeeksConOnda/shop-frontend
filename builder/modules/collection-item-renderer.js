...
class CollectionItemRenderer extends Renderer {
  applyID(template) {
  ...
  applyProductName(template) {
    const { title } = this.options.design;
    return template.replace(/<%PRODUCT_NAME%>/g, title);
  }

  applyProductTypeDescription(template) {
    const { productType } = this.options;
    const { description } = CATEGORIES[productType];
    return template.replace(/<%PRODUCT_TYPE_DESCRIPTION%>/g, description);
  }
}

module.exports = CollectionItemRenderer;
